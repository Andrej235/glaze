...
pub const Wayland = struct {
    ...
    fn frameDone(data: ?*anyopaque, cb: ?*c.struct_wl_callback, _: u32) callconv(.c) void {
        ...
        if (self.frame_event_dispatcher.entries.count() == 0) {
    ...
    pub fn initWindow(width: i32, height: i32, window_title: [*:0]const u8) anyerror!*Window {
        ...
        const fns = struct {
            fn onGlInitializationComplete(wayland: *Wayland, data: ?*anyopaque) anyerror!void {
                ...
                context.* = GlContext{
                    .destroy = fns.destroy,
                    .swapBuffers = fns.swapBuffers,
                    .loadGlad = fns.loadGlad,
                    .data = wayland,
                };

                const gl = try page_allocator.create(Gl);
                gl.* = try Gl.init(context, wayland.win_width, wayland.win_height);
        ...
        const handler_id = try gl_initialization_complete_event_dispatcher.addHandler(fns.onGlInitializationComplete, &res);
        ...
        wl.window = window;
        try wl.gl_initialization_complete_event_dispatcher.removeHandlerById(handler_id);
        return window;
    }
};
