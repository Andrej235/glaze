...
pub const AudioSource = struct {
    ...
    pub fn start(self: *AudioSource) !void {
        ...
        self.sound_group = try self.sound_system.createGroup(.{ .unique_name = group_id });

        self.sounds = try std.ArrayList(*Sound).initCapacity(std.heap.c_allocator, 4);
    }

    pub fn fixedUpdate(self: *AudioSource, _: f32) !void {
    ...
    pub fn playSound(self: *AudioSource, options: SoundOptions) !*Sound {
